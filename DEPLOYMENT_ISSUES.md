...
## Issue 9: Edge Function timed out (504) on registration/login

**Symptom:**
The registration page loads, but clicking the submit button hangs on "Please wait..." and the browser eventually shows a 504 error.

**Root cause:**
The backend was being deployed as a Vercel Edge Function. Edge runtime has strict limits (bundle size, Node.js compatibility, execution time) and the bundled backend either failed silently or could not complete the database request within the Edge environment.

**Fix:**
- Switch back to Vercel's default **Node.js** runtime (remove `export const runtime = "edge"`).
- Write a minimal custom adapter in `api/index.ts` that converts the Node.js `(req, res)` signature into the Web `fetch` API expected by Hono:
  ```ts
  ...
  ```
- Make `server/vercel-entry.ts` export the raw Hono `app` so the adapter can call `app.fetch`:
  ```ts
  import app from "./app";
  export default app;
  ```
